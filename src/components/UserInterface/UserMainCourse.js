...
const UserMainCourse = () => {
    ...
    const mainCourses = items.filter(item => item.category === 'Main Course');
    ...
    return (
        <div style={{ position: "relative", width: "100%", height: "100vh" }}>
            ...
            <div style={{ position: 'absolute', marginTop: '-7%', top: '30%', width: '80%', height: '60%', color: '#81ddf4', fontFamily: 'Cascadia Code Light', textAlign: 'center', marginLeft: '10%' }}>
                <h1 style={{ color: 'whitesmoke', paddingBottom: '20px' }}>Main Courses</h1>
                <Container style={{ color: 'whitesmoke' }}>
                    <Row>
                        {mainCourses.map((Course) => (
                            <Col key={Course.id} xs={3}>
                                <NavLink style={{ textDecoration: 'none', color: 'white' }} to={`/users/maincourse/${encodeURIComponent(Course.name)}`}>
                                    <div style={{ paddingBottom: '10px' }}>{Course.name}</div>
                                    <Image style={{ width: '50%', height: '50%' }} roundedCircle src={Course.image} />
                                </NavLink>
                            </Col>
                        ))}
                    </Row>
