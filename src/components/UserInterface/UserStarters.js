    ...
    return (
        <div style={{ position: "relative", width: "100%", height: "100vh" }}>
            ...
            <div style={{ position: 'absolute', marginTop: '-7%', top: '30%', width: '80%', height: '60%', color: '#81ddf4', fontFamily: 'Cascadia Code Light', textAlign: 'center', marginLeft: '10%' }}>
                <h1 style={{ color: 'whitesmoke', paddingBottom: '20px' }}>Starters</h1>
                <Container style={{ color: 'whitesmoke' }}>
                    <Row>
                        {starters.map((starter) => (
                            <Col key={starter.id} xs={3}>
                                <NavLink style={{ textDecoration: 'none', color: 'white' }} to={`/users/starters/${starter.name}`}>
                                    <div style={{ paddingBottom: '10px' }}>{starter.name}</div>
                                    <Image style={{ width: '50%', height: '50%' }} roundedCircle src={starter.image} />
                                </NavLink>
                            </Col>
                        ))}
                    </Row>
